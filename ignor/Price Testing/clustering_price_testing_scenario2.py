...
output_dir = '/Result/Vertiport_analysis/Probability_clustering/pricetesting_weighting_clustering/scenario2_base5_perkm5'
...
# Save weight history
np.save(os.path.join(output_dir, 'cost2_weight_history.npy'), weight_history)
logger.info(f"Weight history saved: {weight_history.shape}")

# Save UAM probability history
np.save(os.path.join(output_dir, 'cost2_uam_prob_history.npy'), uam_prob_history)
logger.info(f"UAM probability history saved: {uam_prob_history.shape}")

# Save final vertiport coordinates
# Save as .npy
np.save(os.path.join(output_dir, 'cost2_optimized_vertiport_coords.npy'), vertiport_coords)
# Save as .csv
pd.DataFrame(vertiport_coords, columns=['X', 'Y']).to_csv(
    os.path.join(output_dir, 'cost2_optimized_vertiport_coords.csv'), index=False)
...
output_predictions_path = os.path.join(output_dir, 'cost2_LightGBM_synthetic_population_predictions_weights.csv')
...
report_path = os.path.join(output_dir, 'cost2_method_report.txt')
...
try:
    ...
    # Save summary CSV
    summary_path = os.path.join(output_dir, 'cost2_method_summary.csv')
