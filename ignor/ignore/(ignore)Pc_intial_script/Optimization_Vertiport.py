...
# --- Layout similarity ignor functions ---
...
def plot_centroids_and_demand(centroids, origins, destinations, iteration, save_path):
    ...
    plt.scatter(destinations[:, 0], destinations[:, 1], c='lightgreen', s=20, alpha=0.6, label='Destinations', marker='s')
    
    # Plot vertiport centroids
    plt.scatter(centroids[:, 0], centroids[:, 1], c='red', s=100, alpha=0.8, label='Vertiports', marker='^', edgecolors='black', linewidth=1)
...
for iteration in range(max_iter):
    ...
    if prev_coords is not None:
        ...
        min_total_shift = cost_matrix[row_ind, col_ind].sum()  #ignor
