...
# Hyperparameter grid for tuning (if needed)
param_grid = {
...
all_true_labels = []
all_pred_labels = []
importances_list = []  # List to store feature importances for each fold
...
for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X, y), 1):
    ...
    model.fit(X_train_full, y_train_full,
              ...
              early_stopping_rounds=20)  # Stop after 20 rounds of no improvement
...
feature_importance_df.to_csv('aggregated_feature_importances.csv', index=False)
logger.info(f"Aggregated feature importances saved to 'aggregated_feature_importances.csv'")
...
logger.info(f"Predicted probabilities saved to 'predicted_probabilities_LightGBM.csv'")
...
logger.info(f"Aggregated confusion matrix saved to 'confusion_matrix_LightGBM.csv'")
...
logger.info(f"Average metrics saved to 'average_metrics_LightGBM.txt'")