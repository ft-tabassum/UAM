...
def normalize_reduced_model():
    """
    Normalize reduced model data for probability calculations.
    The 'mode' column (target) is not normalized. Missing values and -1 are replaced with 0.
    """
    print("Normalizing reduced model data...")
    # Load data
    data = pd.read_csv('D:/PythonProject/Result/Data_Preprocessing/reduced_data_processed.csv')
    # Replace NaN and -1 with 0
    data = data.fillna(0)
    data = data.replace(-1, 0)
    # Exclude the target column from normalization
    features = data.drop(columns=['mode'])
    # Only keep columns that are present in the new reduced model
    # (all except 'mode')
    scaler = MinMaxScaler()
    ...
    output_path = 'D:/PythonProject/Result/Data_Preprocessing/reduced_model_normalized.csv'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    data_normalized.to_csv(output_path, index=False)
    print(f"Normalized data saved: {output_path}")
    print("Reduced model normalization completed successfully!")
    print(f"Normalized data shape: {data_normalized.shape}")
    print("\nReduced model data statistics after normalization:")
...
if __name__ == "__main__":
    normalized_data, scaler = normalize_reduced_model() 